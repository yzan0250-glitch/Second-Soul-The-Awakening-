/**
 * SECOND SOUL ENGINE — SoulScreen.js
 * Version 4: State-Safe + Debug-Ready
 *
 * Responsibilities:
 *   - Display DAY marker
 *   - Run SecondSoulEngine and display reconstructed structure
 *   - DEV-only export button
 *
 * UI rules:
 *   - Structural text only
 *   - No charts, no scores, no percentages
 *   - Export button visible ONLY in __DEV__ builds
 */
...
export default function SoulScreen({ dayCount = 1 }) {
  ...
  useEffect(() => {
    // Ambient pulse — for not-formed state orb
  ...
  async function loadSoulData() {
    try {
      ...
      const engineSessions = sessions
        .filter(s => s.start_ts && s.end_ts)
        .map(s => ({
          start:    s.start_ts,
          end:      s.end_ts,
          duration: s.duration > 0 ? s.duration : (s.end_ts - s.start_ts),
        }));

      const engineEvents = events.map(e => ({
        timestamp:        e.timestamp,
        session_start:    e.interaction_type === 'session_start',
        session_end:      e.interaction_type === 'session_end',
        app_focus:        e.app_focus,
        interaction_type: e.interaction_type,
        duration:         e.duration,
        app_switch:       !!e.app_switch,
      ...
      // Persist if coherent (identity continuity — never overwrite with silence)
      if (!newInference._not_formed) {
        await persistSoul(newInference, priorSoul);
      }

      // Display: new coherent soul, or prior soul if exists, or not-formed state
      const display = !newInference._not_formed
        ? (await loadSoul() || newInference)
        : (priorSoul || newInference);

      setSoul(display);

      Animated.timing(fadeAnim, { toValue: 1, duration: 1200, useNativeDriver: true }).start();
    } catch (e) {
      console.warn('[SoulScreen] Load error:', e.message);
      setSoul(null);
    } finally {
      setLoading(false);
    }
  ...
  async function handleExport() {
    try {
      ...
      console.log('[DEV] Exported to:', path);
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.completenessRow}>
          ...
          <View>
            <Text style={[styles.completenessText, { color: completenessInfo.color }]}>
              {completenessInfo.en}
            </Text>
...
function SoulSection({ section, value, index, expanded, onToggle }) {
  ...
  return (
    <Animated.View style={[styles.section, { opacity: entryAnim }]}>
      ...
      {expanded && (
        <View style={styles.sectionBody}>{renderValue()}</View>
      )}
...
const styles = StyleSheet.create({
  ...
  notFormedContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 36, gap: 16 },
  ...
  devExportText: { fontSize: 10, letterSpacing: 2, color: COLORS.textDim, textTransform: 'uppercase' },

  footer: { marginTop: 40, paddingTop: 24, borderTopWidth: 1, borderTopColor: COLORS.divider, gap: 12, alignItems: 'center' },
